          ...
          <div className="flex-1 text-center lg:text-left pt-2 sm:pt-4">
            ...
            {profile.subjects && profile.subjects.length > 0 && (
              <div className="text-center lg:text-left mb-3 sm:mb-4">
                ...
                {/* Specialized Topics */}
                {profile.subjects.some(subject => subject.selectedTopics && subject.selectedTopics.length > 0) && (
                  <div className="flex flex-wrap justify-center lg:justify-start gap-1.5">
                                         {profile.subjects.flatMap(subject => 
                       subject.selectedTopics?.map((topic, index) => {
                         const topicName = typeof topic === 'string' ? topic : topic.name;
                         return topicName;
                       }) || []
                     ).slice(0, 6).map((topicName, index) => (
                       <span
                         key={index}
                    ...
                    {profile.subjects.flatMap(subject => 
                      ...
                      <span className="inline-flex items-center px-2 py-1 bg-gradient-to-r from-gray-50 to-gray-100 text-gray-700 text-xs font-medium rounded-lg border border-gray-200/60 shadow-sm">
                        +{profile.subjects.flatMap(subject => 
                          subject.selectedTopics?.map((topic, index) => {
                            const topicName = typeof topic === 'string' ? topic : topic.name;
                            return topicName;
