    ...
    statusFilter === 'all'
      ...
      ? bookings.filter(
          (booking) =>
            ...
            !isSessionOngoing(booking) &&
            !hasSessionEnded(booking)
        )
      : statusFilter === 'ongoing'
      ? bookings.filter(
          (booking) =>
            booking.status === 'confirmed' && isSessionOngoing(booking)
        )
      : statusFilter === 'ended'
      ? bookings.filter(
          (booking) =>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-orange-600">
              {
                bookings.filter(
                  (b) => b.status === 'confirmed' && isSessionOngoing(b)
                ).length
              }
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600">
              {
                bookings.filter(
                  (b) => b.status === 'confirmed' && hasSessionEnded(b)
                ).length
              }
            </div>
